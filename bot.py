...
def generate_basic_horoscope(zodiac_sign, user_id=None):
    ...
    # Фиксируем seed для воспроизводимости в течение дня
    ...
    energy_texts = [
        ...
        "Звезды советуют быть осторожными в финансовых вопросах.",
        "Отличный день для творчества и самовыражения.",
        "Сегодня важно уделить время близким людям.",
        "Энергетический подъем поможет достичь поставленных целей."
    ]
    
    love_texts = [
        "Романтические моменты ждут вас сегодня",
        "Глубокие разговоры укрепят отношения",
        "Будьте открыты новым знакомствам",
        "Вечер обещает приятное общение",
        "Возможна неожиданная встреча из прошлого",
        "День благоприятен для признаний в чувствах"
    ]
    
    career_texts = [
        "Финансовые возможности активизируются",
        "Коллеги окажут поддержку в важном деле",
        "Смелые решения принесут плоды",
        "Возможен карьерный рост или повышение",
        "Удачный день для переговоров",
        "Проявите инициативу в рабочих вопросах"
    ]
    
    health_texts = [
        "Прогулка на свежем воздухе восстановит силы",
        "Обратите внимание на режим сна",
        "Йога или медитация принесут гармонию",
    ...
    advice_texts = [
        "Будьте гибкими в решениях",
        "Отпустите контроль над ситуацией",
        "Проявите терпение — всё придет вовремя",
        "Доверьтесь течению жизни",
